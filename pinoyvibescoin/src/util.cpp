...
const char * const BITCOIN_CONF_FILENAME = "pinoyvibescoin.conf";
const char * const BITCOIN_PID_FILENAME = "pinoyvibescoin.pid";
...
#else
    const char* pszModule = "pinoyvibescoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pinoyvibescoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pinoyvibescoin";
#else
    // Unix
    return pathRet / ".pinoyvibescoin";
#endif
#endif
}
